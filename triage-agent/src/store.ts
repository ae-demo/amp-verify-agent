...
// loaded and saved as a unit. Two backings implement one interface, and the
// configuration chooses between them. Copied from agent-building's
// building.md "Conversation store" — do not redesign the schema or queries.
import pg from "pg";
...
function postgresStore(): ConversationStore {
  ...
  return {
    ...
    // id in the app (crypto.randomUUID()), then upsert. A turn that fails
    // after the id is minted but before the save leaves nothing behind —
    // there is no separate "create the row" step to half-complete.
...
// building.md. It carries the SAME user fence as the SQL: a conversation
// belongs to one user here too, so a behaviour that holds in the cluster is
// the one an evaluation or a local run observes.
...
// Never await initStore() before listen() — see building.md "Never await
// initStore() before listen" for why. initStore() fires the schema init and
// returns immediately; ensureStore() is what every turn awaits, retrying
// until it succeeds.
let ready = false;
